...
const generateUserPayload = (overrides: { email?: string, password?: string, firstName?: string, lastName?: string } = {}) => ({
  ...
  describe('DELETE /user/:id', () => {
    it('should return a status 200 and a deleted user', async() => {
      const res = await request(app)
        ...
        .expect(200)
      expect(new Date(res.body.data.user.deletedAt) instanceof Date).to.equal(true)
    })
  })
})